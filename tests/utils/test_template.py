...
def test_a_portless_instance_renders_empty_rather_than_the_word_none():
    # The one deliberate difference from the old implementation, which did
    # `str(port)` and so produced the literal "None". Both values are broken
    # for a command that wants a port; "" is at least not a plausible-looking
    # token, and nothing can reasonably depend on "None".
...
def test_the_measured_failure_now_renders():
    # The exact value that produced the ZMQError.
...
def test_a_missing_variable_source_is_not_fatal(caplog):
    ...
    # tolerantly for this reason — see the failure it was written against:
    # a caller holding only `_model` used to raise from underneath
    # `_get_configured_env`.
    with caplog.at_level(logging.WARNING):
