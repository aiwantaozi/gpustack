...
class EvaluationClient:
    def __init__(self, client: HTTPClient, enable_cache: bool = True):
        self._client = client
        self._url = "/evaluations"
        self._enable_cache = enable_cache
        self._cache: Dict[int, EvaluationPublic] = {}
    ...
    def list(
        ...
        return EvaluationsPublic.model_validate(response.json())

    def _list_from_cache(self, params: Dict[str, Any] = None) -> EvaluationsPublic:
        """
        ...
        return EvaluationsPublic(items=all_items, total=total, pagination=pagination)
    ...
    def _update_cache_from_event(self, event: Event):
        ...
        try:
            item = EvaluationPublic.model_validate(event.data)
            ...
            with self._get_cache_lock():
                if event.type == EventType.DELETED:
                    ...
                    logger.debug(f"Cache: removed evaluation {item.id}")
                else:  # CREATED or UPDATED
                    self._cache[item.id] = item
                    logger.trace(f"Cache: updated evaluation {item.id}")
        except Exception as e:
            logger.error(f"Failed to update evaluations cache from event: {e}")
    ...
    async def awatch(
        ...
        if self._enable_cache and not self._watch_started:
            ...
            logger.debug(f"evaluations cache watch started")
                        ...
                        if self._enable_cache:
                            ...
                            if (
                                ...
                                if cache_size > 0:
                                    ...
                                    logger.debug(
                                        f"evaluations cache populated with {cache_size} items"
                                    )
    ...
    def get(self, id: int, use_cache: bool = True) -> EvaluationPublic:
        """
        ...
        if should_use_cache:
            with self._get_cache_lock():
                if id in self._cache:
                    logger.trace(f"Cache hit for evaluation {id}")
                    return self._cache[id]

        # Fall back to API call
        response = self._client.get_httpx_client().get(f"{self._url}/{id}")
        raise_if_response_error(response)
        result = EvaluationPublic.model_validate(response.json())
    ...
    def create(self, model_create: EvaluationCreate):
        ...
        raise_if_response_error(response)
        return EvaluationPublic.model_validate(response.json())

    def update(self, id: int, model_update: EvaluationUpdate):
