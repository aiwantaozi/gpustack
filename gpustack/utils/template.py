...
The env-value case is why this module exists. Substitution used to live inside
`replace_command_param`, which is reached only when a version config has a
`run_command` and *no* `built_in_frameworks` — and "has built_in_frameworks"
is the definition of a built-in backend. So the built-in vLLM and SGLang paths
never got there, and `VLLM_NIXL_SIDE_CHANNEL_HOST={{worker_ip}}` reached the
container verbatim: measured as `ZMQError: No such device
