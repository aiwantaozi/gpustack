...
def test_the_migration_only_writes_values_the_writer_produces():
    ...
    into a state the owner would never choose. The first version of this
    backfill wrote `partial` for a partially-scaled model — a value
    `derive_model_state` never returns for a role-less model, and one the
    servability gate reads as unroutable, so the migration would have taken
    every such model out of service until its first reconcile."""
    import pathlib
    import re
    ...
    path = next(versions.glob("*e1f2a3b4c5d6*add_pd*.py"))
    body = path.read_text()
    backfill = body[body.index("def _backfill_state") : body.index("def upgrade")]
