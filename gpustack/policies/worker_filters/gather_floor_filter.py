...
class GatherFloorFilter(WorkerFilter):
    ...
    **A filter, and only here.** `GroupLocalityScorer` argues at length that a
    filter for the router would let one full worker hold a group hostage, and
    that is right *by default*. It stops being right when the operator has
