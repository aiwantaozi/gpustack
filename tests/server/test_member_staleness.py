...
async def test_recording_the_detected_version_does_not_make_a_member_stale():
    """Measured 2026-08-31: three members started, the engine reported SGLang
    0.5.15.post1, the worker wrote it back, and all three went stale nine
    seconds later with nobody having edited anything. The banner then asked for
    a restart to adopt a value read off that very group."""
    model = _model()
