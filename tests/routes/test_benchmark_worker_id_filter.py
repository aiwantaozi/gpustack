"""`GET /v2/benchmarks?worker_id=N` must actually filter (round-2 D17).

Every worker's 3-second state poll sends `worker_id`, believing it gets back
only its own runs. The parameter was never declared on the route, and FastAPI
drops an unknown query parameter silently rather than rejecting it, so each
worker was handed the whole cluster's RUNNING rows and reconciled all of them.
...
by patching the row to ERROR and tearing the workload down. Measured
2026-09-11: asc-w28 killed b155 at 66.6% while asc-w22, the owner, was still
writing stage files.
