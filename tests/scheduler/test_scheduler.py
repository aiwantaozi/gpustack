...
from gpustack.scheduler.placement_policy import PlacementPolicy
...
async def test_schedule_to_single_worker_single_gpu(config):
    ...
    resource_fit_policy = ResourceFitPolicy(model_instance=mi, model=m)
    placement_policy = PlacementPolicy(model=m, model_instance=mi)
    scheduler = Scheduler(config)

    with (
        patch(
            'gpustack.scheduler.resource_fit_policy.get_worker_model_instances',
            return_value=[],
        ),
        patch(
            'gpustack.scheduler.resource_fit_policy.calculate_model_resource_claim',
...
async def test_schedule_to_single_worker_multi_gpu_with_binpack_spread(config):
    ...
    resource_fit_policy_binpack = ResourceFitPolicy(model_instance=mi_binpack, model=m)
    placement_policy_binpack = PlacementPolicy(model=m, model_instance=mi_binpack)

    resource_fit_policy_spread = ResourceFitPolicy(model_instance=mi_spread, model=m)
    placement_policy_spread = PlacementPolicy(model=m, model_instance=mi_spread)
    ...
    with (
        ...
        patch(
            'gpustack.scheduler.placement_policy.get_model_instances',
            return_value=mis,
        ),
    ):
...
async def test_schedule_to_cpu_with_binpack_spread(config):
    ...
    resource_fit_policy_binpack = ResourceFitPolicy(
        model_instance=mi_binpack, model=m, estimate=es.resource_claim_estimate
    )
    placement_policy_binpack = PlacementPolicy(model=m, model_instance=mi_binpack)
    scheduler = Scheduler(config)
    ...
    with (
        ...
        resource_fit_policy_spread = ResourceFitPolicy(
            model_instance=mi_spread, model=m, estimate=es.resource_claim_estimate
        )
        placement_policy_spread = PlacementPolicy(model=m, model_instance=mi_spread)
...
@pytest.mark.asyncio
async def test_find_scale_down_candidates(config):
    ...
    with (
        ...
        candidates = await scheduler.find_scale_down_candidates(m)
