...
🔴 Written to make a fix verifiable, not to find a bug. A multi-NIC Ascend host
used to refuse to derive `HCCL_SOCKET_IFNAME` and made the operator hand-fill
`--kv-ifname` with the very name the platform already had on `Worker.ifname`;
the control-plane recipes now take that name directly. But the e2e that reads
`HCCL_SOCKET_IFNAME=bond1` off a running container cannot tell the fix from the
workaround — the derived value and the hand-filled one are the same string, and
neither the input nor the rendered env was readable from the server.
