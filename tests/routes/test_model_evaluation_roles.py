"""The shape the deployment form now sends to /model-evaluations.

The roles used to be stripped from the evaluation request because a role held
in FORM shape — override switches, GPU ids still in the cascader's
[worker, gpu] pairs — is rejected outright, and a 422 surfaces as an error
toast over the whole form. The fix was to run the same transform submit runs,
so what these assert is that the transform's OUTPUT parses: a role that
