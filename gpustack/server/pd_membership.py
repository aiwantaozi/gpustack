...
# **202** and does the health-probe and the registration on a queue (measured
# 2026-09-16: first pass reported the new prefill missing, the next pass three
# seconds later found it serving). Reading back immediately would therefore
...
class RegistryEntry(NamedTuple):
    ...
    `id` is separate from the URL because upstream stopped being able to name a
    member by its address: through SGLang v0.5.6 the id *is* the URL, and from
    v0.5.7 it is a generated UUID that only the registry knows (measured
    2026-09-16 — the old shape answers `400 Invalid worker_id (expected UUID)`
    on the new one). Reading the id back off the probe is what lets one
    declaration address both, since the removal path uses whatever this says.
    """
...
async def _remove_stale(
    ...
    for url, entry in current.items():
        if url in wanted:
            continue
        # 🔴 Percent-encoded, and this cost a real bug: on the routers that
        ...
        #
        # 🔴 `{id}` is the other half, and it is what makes one declaration
        # work across a breaking upstream change: SGLang moved the member's
        # identity from the URL to a generated UUID at v0.5.7, and the old
        # path now answers `400 Invalid worker_id (expected UUID)` (measured
        # 2026-09-16 on wheels 0.2.2 and 0.3.2). Taking the id from the probe
        # rather than from a version check is correct on both, because the
        # version that keys on the URL reports the URL as the id.
