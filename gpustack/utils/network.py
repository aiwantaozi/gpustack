...
class PortRangeExhaustedError(Exception):
    ...
    answer — "widen the range" — rather than a bug. The bare
    `Exception("No free port available in the port range.")` this replaced
    sent the operator to the source to work that out.
    """
...
def is_port_available(port: int, host: str = "127.0.0.1") -> bool:
    ...
    Binds first, connects only as a fallback. The connect probe alone — what
    this used to be — only sees a port that is *already LISTENing on `host`*:
