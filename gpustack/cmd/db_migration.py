...
migration_temp_file = "gpustack_migration_temp.db"
...
async def _run(args):
    try:
        if not should_migrate():
            logger.info("Skipping migration as GPUSTACK_MIGRATE_SQLITE_DB is not set.")
            return
...
def should_migrate() -> bool:
    return os.environ.get("GPUSTACK_MIGRATE_SQLITE_DB") is not None
...
async def prepare_env(args):
    ...
    copied_sqlite_path = _copy_sqlite_file(data_dir)
...
def clean_env(args):
    ...
    data_dir = args.data_dir
    prefix = migration_temp_file.split('.')[0]
...
def _copy_sqlite_file(data_dir: str):
    sqlite_path = os.path.join(data_dir, "database.db")
    if os.path.exists(sqlite_path) is False:
        raise FileNotFoundError(f"SQLite database file not found at {sqlite_path}")

    copied_file_path = os.path.join(data_dir, migration_temp_file)
    try:
    ...
    async with AsyncSession(new_engine) as new_sess:
        async with new_sess.begin():
            ...
            if user_cluster_1 is not None:
                await new_sess.delete(user_cluster_1)
            ...
            if user_worker_0 is not None:
                await new_sess.delete(user_worker_0)
