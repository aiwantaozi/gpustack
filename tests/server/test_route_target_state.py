...
The bug these guard was measured on 2026-08-31: a worker went unreachable
during a rolling image update, the model correctly left RUNNING and its route
target was set UNAVAILABLE, and the model was back at RUNNING two minutes
later — but the `modelroutetarget` subscription was re-established sixteen
minutes after that. The recovery event fell in the hole, the model then stayed
RUNNING with nothing left to transition, and the target sat UNAVAILABLE
indefinitely. `/v1/models` returned an empty list while the deployment served
correctly when addressed directly.
