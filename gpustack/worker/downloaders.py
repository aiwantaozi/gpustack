...
class OllamaLibraryDownloader:
    ...
    def download(cls, model_name: str, cache_dir: Optional[str] = None) -> str:
        ...
        with FileLock(lock_filename):
            ...
            blob_url, registry_token = cls.model_url(model_name=model_name)
    ...
    def model_url(cls, model_name: str) -> str:
        ...
        token = cls.get_request_auth_token(manifest_url)
        if token:
    ...
    @classmethod
    def get_request_auth_token(cls, request_url):
        ...
        authz_token = cls.get_registry_auth_token(authn_token)
        if not authz_token:
    ...
    @classmethod
    def get_registry_auth_token(cls, authn_token):
        pri_key = cls.load_sing_key()
        if not pri_key:
            return None
    ...
    @classmethod
    def load_sing_key(cls):
        data_dir = Config.get_data_dir()
        key_dir = os.path.join(data_dir, ".ollama")
