...
# Parse command line arguments to handle spaces correctly
GPUSTACK_ARGS_FILE="/var/lib/gpustack/run/args/gpustack"
mkdir -p "$(dirname "$GPUSTACK_ARGS_FILE")"
...
echo "$_args" > "$GPUSTACK_ARGS_FILE"
chmod 0755 "$GPUSTACK_ARGS_FILE"
