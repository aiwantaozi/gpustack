...
Measured three times out of three, with each hop instrumented: the group reaches
`running` with all three members, the route `<base>:<adapter>` is created and
reports `ready_targets=1`, a chat against the base name answers 200, and a chat
against the adapter name answers 503 `No available workers (all circuits open or
unhealthy)`. Both engines are innocent — prefill and decode each list the
adapter in their own `GET /v1/models` and each answer a direct chat on it. The
same adapter, same worker, same base model, deployed as a plain single instance:
200.

So `lora_list` beside `disaggregation` is refused at admission, and
`roles[].lora_list` — which used to be dropped in silence by pydantic's
`extra="ignore"` — is refused wherever it appears.
"""
...
# --- the role level, which used to say nothing at all ---------------------- #


def test_adapters_declared_on_a_role_are_refused():
    """🔴 Until `RoleSpec.lora_list` existed this was a 200 with the adapters
    dropped by `extra="ignore"`: no error, no warning, and nothing in the
    read-back to distinguish it from a request that never carried them."""
