...
🔴 Written after a live fleet lost a host to silence. Its GPU detection was
fine — two RTX 4090s, reported and idle — and its system detection raised, so
it shipped `WorkerStatus.get_default_status()`: memory 0, CPU 0, OS an empty
string, no filesystems. Nothing anywhere said so. The worker stayed READY,
the UI showed two free cards, and every group placement refused it, because
every role wants some RAM and the host looked like it had none. The reason
existed only in that worker's own log, on a machine the operator had no
reason to open.
