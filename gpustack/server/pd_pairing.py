...
The pairing pre-check used to ask "did both roles write this parameter?" and
pass whenever one of them had not. That is a different question from "do the
two roles run the same value", because a role that writes nothing does not run
nothing -- it runs the engine's default, which is a value like any other. The
gap let through the ordinary way a group is misconfigured: edit prefill, leave
decode alone, and every rule here steps aside.
...
* `--dtype` and `--kv-cache-dtype` default to `auto`, whose meaning is "follow
  ...
  admission path with no session to fetch one -- the same wall
  `kv_transfer_budget` documents next door.
