...
class DatasetClient:
    ...
    def list(
        ...
        return DatasetsPublic.model_validate(response.json())

    def _list_from_cache(
        self, params: Dict[str, Any] = None
    ) -> Optional[DatasetsPublic]:
        """
        ...
        return DatasetsPublic(items=all_items, total=total, pagination=pagination)
        ...
        try:
            ...
            if is_id_only_delete:
                ...
                if item is not None:
                    ...
                    event.data = item
                logger.debug(f"Cache: removed dataset {event.id}")
                return

            item = DatasetPublic.model_validate(event.data)
            ...
            with self._cache_lock:
                if event.type == EventType.DELETED:
                    ...
                    logger.debug(f"Cache: removed dataset {item.id}")
                else:  # CREATED or UPDATED
                    self._cache[item.id] = item
                    logger.trace(f"Cache: updated dataset {item.id}")
        except Exception as e:
            logger.error(f"Failed to update datasets cache from event: {e}")
        ...
        try:
            async with self._client.get_async_httpx_client().stream(
                ...
                if self._enable_cache:
                    ...
                    if first_start:
                        logger.debug(f"datasets cache watch started")
                    ...
                    try:
                        ...
                        if line:
                            ...
                            if self._enable_cache:
                                ...
                                if (
                                    ...
                                    if cache_size > 0:
                                        ...
                                        logger.debug(
                                            f"datasets cache populated with {cache_size} items"
                                        )
                            ...
                            if (
                                ...
                                logger.debug(
                                    f"Skipping callback for ID-only {event.type} event on datasets {event.id}"
                                )
                            elif callback:
    ...
    def get(self, id: int, use_cache: bool = True) -> DatasetPublic:
        ...
        if use_cache and self._enable_cache:
            with self._cache_lock:
                if self._watch_started and id in self._cache:
                    logger.trace(f"Cache hit for dataset {id}")
        ...
        raise_if_response_error(response)
        return DatasetPublic.model_validate(response.json())

    def create(self, model_create: DatasetCreate):
    ...
    def update(self, id: int, model_update: DatasetUpdate):
