...
class CacheServiceController:
    ...
    async def _reconcile_service(self, session: AsyncSession, service: CacheService):
        ...
        namespace = await self._namespace_for_new_instances(session, service, room)
        for component, layout in desired_by_component.items():
            spec = provider.get_component(component) if provider else None
    ...
    async def _namespace_for_new_instances(
        self,
        session: AsyncSession,
        service: CacheService,
        room: Dict[Tuple[str, int], int],
    ) -> Optional[str]:
        """The tenant namespace this service's instance containers live in.

        Stamped onto every instance row at creation and never recomputed — the
        namespace is where the container *is*. It is created up front when
        `room` still has openings, i.e. when the caller is about to create
        instances into it; a pass that creates nothing does not pay for the
        call, and a namespace that cannot be created is left to the Pod
        creation to report (see ``WorkloadNamespaceEnsurer.ensure``).
        """
        namespace = await resolve_workload_namespace(
            session, service.owner_principal_id, service.cluster_id
        )
        if max(room.values(), default=0) > 0:
