...
Measured 2026-09-01: `kv_load_failure_policy` is rendered by exactly one of
the four built-in recipes. On the other three the value is accepted, stored
...
def test_the_rule_is_read_off_the_recipe_not_a_list_of_names():
    """A mode that starts rendering the placeholder is accepted the moment it
    does, with nothing here to remember to update. Pinned by checking that the
    accepted set is exactly the set of recipes carrying the placeholder."""
    from gpustack.routes.models import _KV_LOAD_FAILURE_PLACEHOLDER
    from gpustack.server.pd_mode_catalog import get_pd_mode

    for mode in PDModeEnum:
        recipe = get_pd_mode(mode.value)
        renders = recipe is not None and (
            _KV_LOAD_FAILURE_PLACEHOLDER in recipe.model_dump_json()
        )
        try:
            _reject_a_policy_the_mode_cannot_apply(_spec(mode, "recompute"))
            accepted = True
