...
The constant that used to encode it (200, chosen against `PlacementScorer`'s
100 plus the file-locality tiebreaker's 5) stopped being a bound when
`TopologyProximityScorer` joined with a ceiling of 450. These tests pin the
derivation instead of the number, because the number is what rotted.
"""
...
# The 3P1D board the bug report describes. Worker 1 carries the prefills;
...
async def test_a_scaled_out_prefill_lands_on_the_decode_host_across_the_rack():
    """The bug report's own scenario, end to end on the assembled chain.
...
async def test_one_more_sibling_outweighs_the_whole_rest_of_the_chain():
    """🔴 The regression itself, stated as the invariant rather than a scenario.
    ...
    still has to win on siblings alone. At the hardcoded 200 it did not, which
    is the whole of B2; and any bigger constant written here would fail again
    the next time a scorer joins.

    ⚠️ Deliberately harsher than the real `TopologyProximityScorer` can be on
...
async def test_the_topology_scorers_ceiling_counts_its_rungs_not_its_step():
    """🔴 The one ceiling that is not `_max_score`, and the reason 200 rotted.
...
def test_the_next_scorer_on_the_chain_raises_the_weight_with_no_code_change():
    """🔴 What "derived" buys: the fix for B2 has to survive B2 happening again.
...
# --- the level above affinity: do not split the member (B8) ------------------ #
...
async def test_affinity_may_not_buy_a_member_split_across_machines():
    """🔴 B8, and the reason the first key left the arithmetic.

    Raising the affinity weight so it outranks the rest of the chain (B2) also
    raised it past `TopologyProximityScorer`'s split penalty, which is a fixed
    600 on the built-in rungs: two opposite siblings pay 1112 and buy a member
    spread over two machines. The trade is indefensible at any weight — a split
    member pays a tensor-parallel all-reduce once per layer per token to save a
    KV transfer that runs once per request — and no constant can forbid it,
    because affinity is `max_score` PER sibling and so has no ceiling.
