...
async def test_pd_group_weights_only_the_router():
    ...
    with (
        ...
        destinations, _, groups = await calculate_destinations(
    ...
    assert groups[0].service_names == {f"model-{MODEL_ID}-3.static"}
...
async def test_group_without_a_running_router_weights_nothing():
    ...
    with (
        ...
        destinations, fallback, groups = await calculate_destinations(
            session=None, model_route=ModelRoute(id=1, name="asc-pd")
        )
