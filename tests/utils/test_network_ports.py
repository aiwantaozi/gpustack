...
one base, and the probe was rewritten because the old one could only see a
port that was *already listening on the advertised address* — the two are the
same story from opposite ends, so they are pinned in one file.
"""
...
def test_is_port_available_sees_a_bound_but_unlistened_socket():
    """The case the connect probe was blind to.

    A socket that has bound but not yet called listen() refuses connections
    exactly like a free port does, so the old probe handed the port out and
    the collision surfaced later as an engine wedged in `starting`.
    """
    ...
    try:
        ...
        # What the old implementation saw: nothing to connect to.
...
def test_is_port_available_sees_a_listener_on_another_local_address():
    ...
    The connect probe only ever reaches the address it was given, so a
    server on 127.0.0.1 was invisible when the allocator probed the worker's
