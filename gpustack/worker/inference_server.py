...
class InferenceServer:
    ...
    def start(self):  # noqa: C901
        ...
        ).joinpath(get_llama_box_command(compute_mode))
...
def get_llama_box_command(compute_mode):
    command_map = {
        ("gpu", "Windows", "amd64"): "llama-box-windows-amd64-cuda-12.5.exe",
        ("gpu", "Darwin", "amd64"): "llama-box-darwin-amd64-metal",
        ("gpu", "Darwin", "arm64"): "llama-box-darwin-arm64-metal",
        ("gpu", "Linux", "amd64"): "llama-box-linux-amd64-cuda-12.5",
        ("cpu", "Linux", "amd64"): "llama-box-linux-amd64-avx2",
        ("cpu", "Linux", "arm64"): "llama-box-linux-amd64-neon",
        ("cpu", "Windows", "amd64"): "llama-box-windows-amd64-avx2.exe",
        ("cpu", "Windows", "arm64"): "llama-box-windows-arm64-neon.exe",
    }

    command = get_platform_command(command_map, compute_mode)
    if command == "":
        raise Exception(
...
def get_rpc_servers(model_instance: ModelInstance, worker_map):
    rpc_servers = []
    rpc_tensor_split = []
    if model_instance.distributes_servers and model_instance.distributes_servers.get(
        "rpc_servers"
    ):
        for rpc_server in model_instance.distributes_servers.get("rpc_servers"):
