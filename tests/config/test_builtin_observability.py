...
def test_prepare_prometheus_config_writes_observability_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("GPUSTACK_OBSERVABILITY_DIR", str(observability_dir))
    ...
    env_text = (observability_dir / ".env").read_text()
...
async def test_prometheus_proxy_uses_configured_builtin_port(monkeypatch, tmp_path):
    ...
    assert response == {
        "url": "http://127.0.0.1:19090/prometheus/api/v1/query?query=up"
    }
    assert captured["url"] == "http://127.0.0.1:19090/prometheus/api/v1/query?query=up"
